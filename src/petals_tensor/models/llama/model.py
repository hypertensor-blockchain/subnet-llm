...
from petals_tensor.client.from_pretrained import FromPretrainedMixin
...
from petals_tensor.validator.from_pretrained import FromPretrainedMixin as FromPretrainedMixinValidator
from petals_tensor.validator.lm_head import LMHead as LMHeadValidator
from petals_tensor.validator.ptune import PTuneMixin as PTuneMixinValidator
from petals_tensor.validator.remote_generation import RemoteGenerationMixin as RemoteGenerationMixinValidator
from petals_tensor.validator.remote_generation import RemotePastKeyValues as RemotePastKeyValuesValidator
from petals_tensor.validator.remote_sequential import RemoteSequential as RemoteSequentialValidator

from petals_tensor.models.llama.config import DistributedLlamaConfig, DistributedLlamaValidatorConfig
...
class DistributedLlamaModelValidator(FromPretrainedMixinValidator, PTuneMixinValidator, LlamaModel):
    ...
    config_class = DistributedLlamaValidatorConfig

    def __init__(self, config: DistributedLlamaValidatorConfig, *, dht: Optional[hivemind.DHT] = None):
        n_layer, config.num_hidden_layers = config.num_hidden_layers, 0  # Prevent initialization
        super().__init__(config)
        assert len(self.layers) == 0
        config.num_hidden_layers = n_layer
...
class DistributedLlamaForCausalLMValidator(FromPretrainedMixinValidator, RemoteGenerationMixinValidator, LlamaForCausalLM):
    ...
    def __init__(self, config: DistributedLlamaValidatorConfig):
        LlamaPreTrainedModel.__init__(self, config)
